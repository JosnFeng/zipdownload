...
- (void)sendData:(id)data responseCallback:(WVJBResponseCallback)responseCallback handlerName:(NSString*)handlerName {
    ...
    if (responseCallback) {
        ...
        [message setValue:callbackId forKey:@"callbackId"];
    }
    
    if (handlerName) {
        [message setValue:handlerName forKey:@"handlerName"];
    }
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJBMessage* message in messages) {
        ...
        NSString* responseId = [message getStringForKey:@"responseId"];
        if (responseId) {
            WVJBResponseCallback responseCallback = _responseCallbacks[responseId];
            responseCallback([message getValueForKey:@"responseData"]);
            [self.responseCallbacks removeObjectForKey:responseId];
        } else {
            WVJBResponseCallback responseCallback = NULL;
            NSString* callbackId = [message getStringForKey:@"callbackId"];
            if (callbackId) {
            ...
            handler([message getValueForKey:@"data"], responseCallback);
        }
    }
}
